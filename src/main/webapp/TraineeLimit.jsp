...
<title>Set up for trainee Limit</title>
<!-- Favicon-->
<!-- Include jQuery -->
...
<body>
	...
	<a href="/AddTraineeLimit.jsp" class="btn btn-success mt-2">Add Component Resource +</a>
	...
	<script>
		...
		function displayTrainingData(data) {
			...
			for (var i = 0; i < trainings.length; i++) {
				...
				row.append("<td>" + training.expense_type + "</td>");
				row.append("<td>" + training.budgeted_year + "</td>");
				row.append("<td>" + training.trainee_role + "</td>");
		...
		function deleteTraining(button,vid) {
			...
			$.ajax({
				...
				success : function(response) {
					...
					toastr.success("Component Resource deleted successfully");
					// Reload or update the data after deletion
					fetchDataAndDisplay();
				},
				error : function(xhr, status, error) {
					toastr.error("Failed to delete Component Resource: " + error);
				}
			});
		}
		...
		function editTraining(sl) {
			...
			var editUrl = "/EditVenue.jsp?sl=" + sl;// Redirect to the EditTraining.jsp page with the query string parameters
