...
public interface SearchVenueRepository extends JpaRepository<SearchVenue, String> {
	...
	int updateVenue(@Param("vid") String vid, @Param("vname") String vname, @Param("vstate") String vstate,
			@Param("vmandal") String vmandal, @Param("vdistrict") String vdistrict,
			@Param("vlocation") String vlocation, @Param("vcapacity") String vcapacity,
			@Param("vaddress") String vaddress, @Param("landmark") String landmark, @Param("vtype") String vtype,
			@Param("vcontactname") String vcontactname, @Param("vcontactno") String vcontactno,
	...
	@Query("SELECT MAX(SUBSTRING(e.vid, 3)) FROM SearchVenue e WHERE e.vid LIKE 'v-%'")
	Long findHighestId();
